...
// @author      Ryan Harter
// @attribution shellshock (http://userscripts.org/scripts/review/57672), Nicky Leach (http://userscripts.org/scripts/show/56774), Tom Hensel (http://userscripts.org/scripts/review/72181)
...
(function () {
    ...
    var sound = "Blow";// The System Sound to play
	...
	var frame = document.getElementsByClassName("TK").item(0);
	if( frame ) {
	    var inboxElement = frame.getElementsByClassName("n0").item(0);
	...
	if (unreadMsgCount > oldCount) {
	    if (sound) {
		window.fluid.playSound(sound);
	    } else {
		// Play default system alert sound (see OSX "Sound" Preferences Pane)
		window.fluid.beep();
	    }
	    // Show Growl notification
	    var description = unreadMsgCount + ' unread message';
	    if (unreadMsgCount > 1) {
		description += 's';
	    }
